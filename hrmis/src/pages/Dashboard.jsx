...
export default function Dashboard() {
  ...
  // ✅ menuItems role ke hisaab se filter honge
  const getMenuItems = (role) => {
    const common = [{ icon: faHome, label: "Dashboard", path: "/dashboard" }];
    ...
    if (role === "employee") {
      return [
        ...common,
        { icon: faCalendar, label: "My Leaves", path: "/leaves" },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resProfile = await API.get("/users/profile");
        const role = resProfile.data.role;
        const userId = resProfile.data.id;

        // ✅ Employees sirf admin/manager ke liye
        ...
        // ✅ Leaves role ke hisaab se
        let resLeavesPending, resLeavesApproved, resLeavesRejected;
        if (role === "employee") {
          resLeavesPending = await API.get(`/leaves/user/${userId}/pending`);
          resLeavesApproved = await API.get(`/leaves/user/${userId}/approved`);
          resLeavesRejected = await API.get(`/leaves/user/${userId}/rejected`);
        } else {
          resLeavesPending = await API.get(`/leaves/pending`);
          resLeavesApproved = await API.get(`/leaves/approved`);
          resLeavesRejected = await API.get(`/leaves/rejected`);
        }

        // ✅ Tasks role ke hisaab se
        let resTasks;
        if (role === "employee") {
          resTasks = await API.get(`/tasks/user/${userId}`);
        } else {
          resTasks = await API.get("/tasks");
        }
  ...
  return (
    <div className="dashboard-container">
      ...
      <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
        ...
        <ul className="menu-list">
          {getMenuItems(profile.role).map((item, index) => (
            <li
              key={index}
              className="menu-item"
