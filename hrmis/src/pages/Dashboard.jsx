...
import logoImg from "../assets/logo.png";
import UserManagement from "../components/UserManagement"; // 🔗 Import user component
import "./Dashboard.css";

export default function Dashboard() {
  const navigate = useNavigate();
  const [collapsed, setCollapsed] = useState(false);
  const [activeView, setActiveView] = useState("dashboard"); // 🔸 New state for switching views
  const [profile, setProfile] = useState({});
  const [stats, setStats] = useState({
    totalEmployees: 0,
    pendingLeaves: 0,
  ...
  const menuItems = [
    { icon: faHome, label: "Dashboard", view: "dashboard" },
    ...
    <div className="dashboard-container">
        ...
        <ul className="menu-list">
          {menuItems.map((item, index) => (
            <li
              ...
              onClick={() => {
                if (item.view) {
                  setActiveView(item.view); // 🔸 Switch to Employees or Dashboard
                } else {
                  navigate(item.path);
                }
              }}
      ...
      <main className={`main-content ${collapsed ? "sidebar-collapsed" : ""}`}>
        {activeView === "employees" ? (
          // 🔸 Show user management when Employees selected
          <UserManagement />
        ) : (
          <>
            <div className="admin-card">
